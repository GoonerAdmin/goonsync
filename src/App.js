...
import { Users, TrendingUp, History, LogOut, Plus, Link2, Play, Square, Trophy, Crown, Menu, X, Mail, ArrowRight } from 'lucide-react';
...
const App = () => {
  ...
  useEffect(() => {
    ...
    return () => { subscription.unsubscribe(); };
  }, [user, circles, loadActiveUsers]);

  useEffect(() => { if (user) { loadCircles(); loadSessions(); } }, [user, view, loadCircles, loadSessions]);
  useEffect(() => { if (circles.length > 0) loadActiveUsers(); }, [circles, loadActiveUsers]);
